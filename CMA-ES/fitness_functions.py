...
class FitnessFunctions(object):  # TODO: this class is not necessary anymore? But some effort is needed to change it
    ...
    def Myfit(self, x):
        ...
        y_data = [i * 2 + 1 for i in x_data]
        x = [x] if isscalar(x[0]) else x  # scalar into list
        x = np.asarray(x)
        y_c = [i * x[0][0] + x[0][1] for i in x_data]
        f = sum([(x - y) * (x - y) for x, y in zip(y_c, y_data)])

        return f   # 1-element-list into scalar



    def Myfit1(self,x,x_train = x_train,y_train = y_train,y_test = y_test):
        """My objective function"""
        ...
        f = 0
        for puf in range(64):
            y_c = [ sum([i[j] * x[0][65*puf+j] for j in range(64) ] ) + x[0][65*puf+64]   for i in x_train]
            pred = np.sign(y_c)
            f = f + sum([abs(x - y) for x, y in zip(pred, y_train[0:train_num,puf])])
        #y_c = [1/(1+(sum([i[j] * x[0][j] + x[0][64] for j in range(64)]))) for i in x_train]
        ...
        #print('准确率等于：' , cout/train_num)

        #f = sum([- y * cmath.log(x) - (1 - y) * cmath.log(1 - x)  for x,y in zip(y_c,y_train)])/train_num
        return f  # 1-element-list into scalar
