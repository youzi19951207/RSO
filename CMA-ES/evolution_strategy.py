...
'''------------------------------------------------------My change--------------------------------------------------------------------------'''
CH = np.load("F:/万露/万露的实验2/CMA/#64_b64_100000_challenge_8XOR.npy")
# RSP=np.load('G:/Data/Data64_error/#1_b64_100000_response_10%.npy').transpose()
RSP = np.load('F:/万露/万露的实验2/CMA/#64_b64_100000_response_8XOR.npy')
NUM_CHBIT = 64
train_num = 100
X_train = CH
temp = 1 - 2 * X_train
...
y_train = np.array(RSP[0:64, 0:train_num]).transpose()
y_test = np.array(RSP[0:64, 90000:100000]).transpose()
y_test = 2 * y_test - 1
y_train = 2 * y_train - 1
'''------------------------------------------------------------------------------------------------------------------------------------------'''
...
#    'CMA_activefac': '1  # learning rate multiplier for active update',
    ...
    'tolfun': '0  #v termination criterion: tolerance in function value, quite useful',
    'tolfunhist': '0  #v termination criterion: tolerance in function value history',
    'tolstagnation': '10000000000  #v termination if no improvement over tolstagnation iterations',
    ...
    def disp(self, modulo=None):
        ...
        if modulo:
            ...
            if self.countiter > 0 and (self.stop() or self.countiter < 4
                ...
                '''---------------------------------------------------My change-----------------------------------------------------'''
                xx = self.result.xbest
                print("--------------------------------")
                cout1 = 0
                cout = 0
                for puf in range(64):
                    y_c1 = [sum([i[j] * xx[65*puf + j] for j in range(64)]) + xx[65*puf + 64] for i in x_train]
                    pred1 = np.sign(y_c1)

                    for i in range(train_num):
                        if (pred1[i] == y_train[i,puf]):
                            cout1 = cout1 + 1

                    y_c = [sum([i[j] * xx[65*puf + j]  for j in range(64)]) + xx[65*puf + 64] for i in x_test]
                    pred = np.sign(y_c)

                    for i in range(10000):
                        if (pred[i] == y_test[i,puf]):
                            cout = cout + 1

                print('测试集准确率为：', cout /640000,"训练集集准确率为:"  , cout1 / (train_num*64))
        ...
        if 1 < 3 or len(self): # only if another termination criterion is satisfied
            ...
            if 1 < 3:  # add stop condition, in case
                self._addstop('flat fitness',  # message via stopdict
                     ...
                     es.best.f < 1e-1 ,"please (re)consider how to compute the fitness more elaborately if sigma=%.2e is large" % es.sigma)
            '''-----------------------------------------------------------------------------------------------------------------------'''
